"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.NominationsController = void 0;
const nomination_service_1 = require("../services/nomination.service");
const create_nomination_dto_1 = require("../dto/create-nomination.dto");
const current_user_decorator_1 = require("../../auth/decorators/current-user.decorator");
const common_1 = require("@nestjs/common");
const index_1 = require("@prisma/client/index");
const platform_express_1 = require("@nestjs/platform-express");
let NominationsController = class NominationsController {
    nominationService;
    constructor(nominationService) {
        this.nominationService = nominationService;
        console.log('Instantiating Nominations Controller');
    }
    ;
    async create(createNominationDto, file) {
        console.log('Received nomination request');
        try {
            const nomination = await this.nominationService.createNomination(createNominationDto, file);
            return {
                success: true,
                data: nomination,
                message: 'Nomination created successfully',
            };
        }
        catch (error) {
            console.error(`Failed to create nomination: ${error.message}`, error.stack);
            if (error instanceof common_1.BadRequestException) {
                throw error;
            }
            throw new common_1.InternalServerErrorException('Internal server error');
        }
    }
    async getMyNominations(user) {
        return this.nominationService.findAll({
            aspirantId: user.id,
        });
    }
    async getStatistics() {
        return this.nominationService.getStatistics();
    }
    async getNominationsByPosition(position) {
        return this.nominationService.getNominationsByPosition(position);
    }
    async getNomination(id) {
        return this.nominationService.findOne(id);
    }
    async getAllNominations(status, position) {
        return this.nominationService.findAll({
            status,
            position,
        });
    }
};
exports.NominationsController = NominationsController;
__decorate([
    (0, common_1.Post)(),
    (0, common_1.UseInterceptors)((0, platform_express_1.FileInterceptor)('photo')),
    __param(0, (0, common_1.Body)()),
    __param(1, (0, common_1.UploadedFile)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_nomination_dto_1.CreateNominationDto, Object]),
    __metadata("design:returntype", Promise)
], NominationsController.prototype, "create", null);
__decorate([
    (0, common_1.Get)('my-nominations'),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], NominationsController.prototype, "getMyNominations", null);
__decorate([
    (0, common_1.Get)('statistics'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], NominationsController.prototype, "getStatistics", null);
__decorate([
    (0, common_1.Get)('by-position/:position'),
    __param(0, (0, common_1.Param)('position')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], NominationsController.prototype, "getNominationsByPosition", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], NominationsController.prototype, "getNomination", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Query)('status')),
    __param(1, (0, common_1.Query)('position')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", Promise)
], NominationsController.prototype, "getAllNominations", null);
exports.NominationsController = NominationsController = __decorate([
    (0, common_1.Controller)('nominations'),
    __metadata("design:paramtypes", [nomination_service_1.NominationService])
], NominationsController);
//# sourceMappingURL=nomination-submission.controller.js.map