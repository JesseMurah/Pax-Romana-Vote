...
export class ResultsService {
    ...
    async getResultsSummary(useCache = true): Promise<ResultSummary> {
        ...
        if (summary.totalEligibleVoters > 0) {
            summary.overallTurnout = Math.round(
     ...
     */
    async getPositionResults(position: Candidate_Position): Promise<PositionResult> {
     ...
     */
    async exportResultsAsPDF(positions?: Candidate_Position[],
                             certifiedOnly = false,
                             includeAuditTrail = false): Promise<Buffer> {
     ...
     */
    async exportResultsAsJSON(positions?: Candidate_Position[],
                              certifiedOnly = false,
                              includeAuditTrail = false): Promise<Buffer> {
        const exportOptions: ExportOptionsDto = {
     ...
     */
    async exportResultsAsCSV(positions?: Candidate_Position[],
                             certifiedOnly = false,
                             includeAuditTrail = false): Promise<Buffer> {
        const exportOptions: ExportOptionsDto = {
     ...
     */
    async recountPosition(position: Candidate_Position, triggeredBy: string): Promise<PositionResult> {
    ...
    async generateElectionStatistics(): Promise<any> {
        ...
        const stats = {
            ...
            competition: {
                ...
                averageCandidatesPerPosition: summary.positionResults.reduce(
     ...
     */
    async generateResultsSnapshot(format: ExportFormat = ExportFormat.JSON): Promise<Buffer> {
     ...
     */
    async exportPositionResults(positions: Candidate_Position[],
                                format: ExportFormat = ExportFormat.PDF): Promise<Buffer> {
