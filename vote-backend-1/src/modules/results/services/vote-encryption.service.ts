...
export class VoteEncryptionService {
    ...
    decryptVote(encryptedVote: string): Record<Candidate_Position, string> {
        try {
            ...
            const encryptedData: EncryptedVoteData = JSON.parse(encryptedVote);
            ...
            for (const [positionStr, candidateId] of Object.entries(voteData.selections)) {
                ...
                if (Object.values(Candidate_Position).includes(positionStr as Candidate_Position)) {
                    selections[positionStr as Candidate_Position] = candidateId as string;
                }
            }
     ...
     */
    private async createDecryptionErrorLog(voteId: string, errorMessage: string): Promise<void> {
        try {
