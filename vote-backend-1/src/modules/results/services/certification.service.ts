...
export class CertificationService {
    ...
    ): Promise<{ success: boolean; certifiedPositions: string[]; errors: string[] }> {
    ...
    private async certifyPosition(
        ...
        if (positionResult.status === ResultStatus.DISPUTED) {
            throw new BadRequestException(`Position ${position} has disputed results and cannot be certified`);
        }
